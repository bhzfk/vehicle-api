...
var test1 = router.route("/api/test1");
test1.get(function(req, res, next) {
            ...
            if (results[0] != null) {
                if (passWord == results[0].password) {
                    if (results[0].role == "owner") {
                        ...
                        res.end(JSON.stringify({
                            state: false,
                            message: "Not Access To This Area",
                            token: "",
                        }));
...
checkState.put(function(req, res, next) {
    ...
    let userId = 0;
    // res.setHeader("Content-Type", "application/json");
    // res.end(JSON.stringify({ state: true, message: "asdadadd" }));
    submitTracking(req.body.token, "update checkstate", req, req.body.vehicleId)
        .then((data) => {
            userId = data.data.id;
            // res.setHeader("Content-Type", "application/json");
            // res.end(JSON.stringify({ state: true, message: "uuuuuuuuu" }));
            req.getConnection(function(err, conn) {
                if (err) return next("Cannot Connect");
                ...
                var query = conn.query(queryTxt, function(err, results) {
                    if (true || results.length === 0 || req.body.status === "partOk") {
                        ...
                        queryTxt = queryTxt + `modifireId = '${userId}', tiresSize = '${tiresSize}', tiresAge = '${tiresAge}'  WHERE checkpointId = ${req.body.checkpointId} and vehicleId=${req.body.vehicleId}`;
...
megaActionCheckpoints.post((req, res) => {
    if (!req.body.vehicleId) {
        ...
        res.json({
            status: false,
            message: "Checkpoint type is required",
        });
    } else {
        req.getConnection(function(err, conn) {
...
cat.get(function(req, res, next) {
    ...
    req.getConnection(function(err, conn) {
        ...
        var query = conn.query("SELECT post.*,admins.gender,admins.name,admins.avatar,cat.alias FROM `post` JOIN admins ON post.creatorID=admins.id join cat on post.catID = cat.id where cat.title = ? ORDER BY createDate DESC LIMIT 3", cat_alias, function(err, rows) {
            ...
            res.render("cat", {
                title: "دسته" + " " + cat_alias,
                catTitle: cat_alias.split("-").join(" "),
                data: rows,
            });
        });
    });
...
function sendSparePartsEmail(email) {
    let transporter = nodemailer.createTransport({
        host: "mail.fahrzeugtool.inosup.org",
        port: 587,
